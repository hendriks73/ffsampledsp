...
public class TestFFURLInputStream {
  ...
  @Test
  public void testReadThrough48kAppleLosslessFile()
    ...
    try {
      ...
      for (int i = 0; i < stemCount; i++) {
        ...
        try {
          ...
          final AudioInputStream pcmIn =
              AudioSystem.getAudioInputStream(AudioFormat.Encoding.PCM_SIGNED, in);
          stems[i] =
              File.createTempFile(
                  "testSplitStemFile", filename.replace(".stem.mp4", "." + stemNames[i] + ".wav"));
      ...
      AudioSystem.write(
          new AudioInputStream(
              new ByteArrayInputStream(
                  new byte[] {
                    (byte) first, (byte) pattern, (byte) pattern,
                  }),
              new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, 44100, 24, 1, 3, 44100, false),
              1),
  ...
  private int readThroughFile(final String prefix, final String filename)
    ...
    try (final FFAudioInputStream in =
        new FFAudioInputStream(
            new FFURLInputStream(file.toURI().toURL()),
  ...
  private int skipThroughFile(final String prefix, final String filename)
      throws IOException, UnsupportedAudioFileException {
    final File file = File.createTempFile(prefix, filename);
    extractFile(filename, file);
    int bytesSkipped = 0;
