...
/**
 * Our version of {@link AudioFileFormat}, instantiated from native code in <code>
 * FFAudioFileReader.c</code>.
 *
...
public class FFAudioFileFormat extends AudioFileFormat {
  ...
  static {
    TYPE_MAP.put(
        FFAudioFormat.FFEncoding.Codec.MP1.getEncoding().getCodecId(), FFAudioFileFormat.MP1);
    TYPE_MAP.put(
        FFAudioFormat.FFEncoding.Codec.MP2.getEncoding().getCodecId(), FFAudioFileFormat.MP2);
    TYPE_MAP.put(
        FFAudioFormat.FFEncoding.Codec.MP3.getEncoding().getCodecId(), FFAudioFileFormat.MP3);
    TYPE_MAP.put(
        FFAudioFormat.FFEncoding.Codec.FLAC.getEncoding().getCodecId(), FFAudioFileFormat.FLAC);
    TYPE_MAP.put(
        FFAudioFormat.FFEncoding.Codec.VORBIS.getEncoding().getCodecId(), FFAudioFileFormat.VORBIS);
    // TODO: add more?
  }
  ...
  public FFAudioFileFormat(
    ...
    super(
        getAudioFileFormatType(url, codecId),
        getLength(url),
        new FFAudioFormat(
            codecId,
            sampleRate,
            ...
            channels,
            frameSize,
            determineFrameRate(codecId, sampleRate, frameRate),
            bigEndian,
            bitRate,
            vbr,
            encrypted),
  ...
  private static int determineFrameLength(
      final float frameRate, final long durationInMicroSeconds) {
  ...
  private static float determineFrameRate(
      final int codecId, final float sampleRate, final float frameRate) {
  ...
  private static Type getAudioFileFormatType(final String url, final int codecId)
      throws UnsupportedAudioFileException {
    if (url == null) {
