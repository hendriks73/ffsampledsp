...
public class FFAudioInputStream extends AudioInputStream {
  ...
  private static long getFrameLength(final AudioFormat format, final long frameLength) {
    ...
    return frameLength > 0 && format.getFrameSize() > 0 ? frameLength : AudioSystem.NOT_SPECIFIED;
  }
  ...
  /**
   * Indicates whether this stream is seekable. Typically, stream based streams (as opposed to
   * file-based streams) are not seekable.
   *
