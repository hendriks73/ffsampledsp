...
public class FFFormatConversionProvider extends FormatConversionProvider {
  ...
  public AudioFormat.Encoding[] getSourceEncodings() {
    ...
    final Set<FFAudioFormat.FFEncoding> supportedAudioFormats =
  ...
  public AudioFormat.Encoding[] getTargetEncodings() {
    ...
    return new AudioFormat.Encoding[] {
      PCM_SIGNED.getEncoding(), PCM_UNSIGNED.getEncoding(), PCM_FLOAT.getEncoding()
    };
  }
  ...
  public boolean isConversionSupported(
    ...
    // because we only support conversions from FFAudioInputStreams we have to check whether the
    // source format
    // was created by us. All source formats created by us, have the property "provider" set to
    // "ffsampledsp".
    if (!FFAudioFormat.FFSAMPLEDSP.equals(sourceFormat.properties().get(FFAudioFormat.PROVIDER)))
      return false;
  ...
  public AudioFormat[] getTargetFormats(
    ...
    if (PCM_UNSIGNED.getEncoding().equals(ffEncoding)
        || PCM_SIGNED.getEncoding().equals(ffEncoding))
  ...
  public AudioInputStream getAudioInputStream(
    ...
    try {
      return new FFAudioInputStream(
          new FFCodecInputStream(targetFormat, (FFAudioInputStream) sourceStream),
          targetFormat,
          AudioSystem.NOT_SPECIFIED);
    } catch (Exception e) {
