...
/**
 * First tries to load a library the default way using {@link System#loadLibrary(String)}, upon
 * failure falls back to the base directory of the given class package or the jar the class is in.
 * This way, a native library is found, if it is located in the same directory as a particular jar,
 * identified by a specific class from that jar.
 *
...
public final class FFNativeLibraryLoader {
  ...
  private static final String HOST =
      OS_NAME.contains("mac") ? "macos" : (OS_NAME.contains("win") ? "win" : "linux");
  ...
  public static synchronized boolean loadLibrary() {
    ...
    try {
      ...
      LOG.severe(
          "Failed to load native library 'ffsampledsp'. Please check your library path. FFSampledSP will be dysfunctional.");
    }
  ...
  /**
   * Loads a library. First tries {@code libname-arch-host}, via the library path, then {@code
   * (lib)libname-arch-*.ext} via the classpath, then {@code libname} via the library path, and
   * finally {@code (lib)libname*.ext} via the classpath.
   *
   * @param libName name of the library, as described in {@link System#loadLibrary(String)} );
  ...
  public static synchronized void loadLibrary(final String libName, final Class<?> baseClass) {
    ...
    final String packagedNativeLib =
        libName + "-" + ARCH + "-" + HOST + "." + NATIVE_LIBRARY_EXTENSION;
    // but extracted, we want to keep things separate
    final String extractedNativeLibFilename =
        libName + "-" + ARCH + "-" + HOST + "-" + VERSION + "." + NATIVE_LIBRARY_EXTENSION;
    if (LOG.isLoggable(Level.FINE)) {
      LOG.fine("packagedNativeLib: " + packagedNativeLib);
    }
    final File extractedNativeLib =
        new File(System.getProperty("java.io.tmpdir") + "/" + extractedNativeLibFilename);
  ...
  /**
   * Extracts the given resource and writes it to the specified file. Note that this method fails
   * silently.
   *
  ...
  private static void extractResourceToFile(
      final Class<?> baseClass, final String sourceResource, final File targetFile) {
    try (final InputStream in = baseClass.getResourceAsStream(sourceResource)) {
      if (in != null) {
        ...
        if (LOG.isLoggable(Level.FINE))
          LOG.fine("Created " + targetFile + " from resource " + sourceResource);
      } else {
      ...
      LOG.log(
          Level.WARNING,
          "Failed to extract native lib using base class "
              + baseClass
              + " and resource name "
              + sourceResource
              + " to file "
  ...
  public static String findFile(
      final String name, final Class<?> baseClass, final FileFilter filter)
      throws FileNotFoundException {
    try {
      final String filename;
      final String fullyQualifiedBaseClassName = baseClass.getName();
      final String baseClassName =
          fullyQualifiedBaseClassName.substring(fullyQualifiedBaseClassName.lastIndexOf('.') + 1);
          ...
          directory =
              new File(
                  path.substring(
                      0,
                      path.length()
                          - fullyQualifiedBaseClassName.length()
  ...
  private static class LibFileFilter implements FileFilter {
    ...
    public boolean accept(final File file) {
      ...
      return file.isFile()
          && (fileName.startsWith(libName + arch)
              || fileName.startsWith(NATIVE_LIBRARY_PREFIX + libName + arch))
  ...
  /**
   * Decode % encodings in URLs. The common {@link java.net.URLDecoder#decode(String, String)}
   * method also converts {@code +} to {@code space}, which is not what we want.
   *
   * @param s url
   * @return decoded URL
   */
    ...
    while (i < numChars) {
      ...
      if (c == '%') {
        ...
        try {
          ...
          while (((i + 2) < numChars) && (c == '%')) {
            ...
            if (v < 0) {
              throw new IllegalArgumentException(
                  "FFNativeLibraryLoader: Illegal hex characters in escape (%) pattern - negative value");
            }
            bytes[pos++] = (byte) v;
            i += 3;
            if (i < numChars) {
          ...
          if ((i < numChars) && (c == '%')) {
            throw new IllegalArgumentException(
                "FFNativeLibraryLoader: Incomplete trailing escape (%) pattern");
          }

          sb.append(new String(bytes, 0, pos, UTF_8));
        } catch (NumberFormatException e) {
          throw new IllegalArgumentException(
              "FFNativeLibraryLoader: Illegal hex characters in escape (%) pattern - "
                  + e.getMessage());
        }
        needToChange = true;
      } else {
