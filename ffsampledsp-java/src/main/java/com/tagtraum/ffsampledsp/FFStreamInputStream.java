...
public class FFStreamInputStream extends FFNativePeerInputStream {
  ...
  /**
   * Always returns <code>false</code>. Stream based {@link FFNativePeerInputStream}s are not
   * seekable.
   *
   * @return false
   */
  ...
  /**
   * Tells native code to fill the {@link #nativeBuffer} with decoded audio. It does so by first
   * calling {@link #fillReadBuffer()} to read encoded audio, decodes it, and then places the
   * decoded data into {@link #nativeBuffer}.
   *
