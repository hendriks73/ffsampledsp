...
static const int CALLBACK_BUFFERSIZE = 32 * 1024;
...
static void init_ids(JNIEnv *env) {
  ...
  if (!limit_MID) {
    ...
    ffAudioFileFormat_class =
        (*env)->FindClass(env, "com/tagtraum/ffsampledsp/FFAudioFileFormat");

#ifdef DEBUG
    if (!bufferClass)
      fprintf(stderr, "Failed to find java/nio/Buffer\n");
    if (!ffAudioFileFormat_class)
      fprintf(stderr,
              "Failed to find com/tagtraum/ffsampledsp/FFAudioFileFormat\n");
#endif

    limit_MID = (*env)->GetMethodID(env, bufferClass, "limit", "()I");
    ffAudioFileFormat_MID = (*env)->GetMethodID(
        env, ffAudioFileFormat_class, "<init>",
        "(Ljava/lang/String;IFIIIFIZJILjava/lang/Boolean;Z)V");
#ifdef DEBUG
    if (!limit_MID)
      fprintf(stderr, "Failed to find limit method\n");
    if (!ffAudioFileFormat_MID)
      fprintf(stderr, "Failed to find constructor of AudioFileFormat\n");
#endif
  }
}
 ...
 * @return          number of bytes written to buf or a negative value in case
 * of an error
 */
static int read_callback(void *opaque, uint8_t *buf, int size) {
  ...
  java_buffer =
      (uint8_t *)(*callback->env)
          ->GetDirectBufferAddress(callback->env, callback->byte_buffer);
  if (!java_buffer) {
    res = -1;
    throwIOExceptionIfError(callback->env, 1,
                            "Failed to get address for byte buffer");
    goto bail;
  }
  // get available data, i.e. the limit of the java buffer
  availableData =
      (*callback->env)
          ->CallIntMethod(callback->env, callback->byte_buffer, limit_MID);
  // copy to c buf
...
/**
 * Determines, whether the given codec is a standard, interleaved PCM codec or
 * not.
 *
 * @param id    AVCodecID
...
static jlong duration(AVFormatContext *format_context, AVStream *stream) {
  ...
  if (format_context->duration != AV_NOPTS_VALUE) {
    ...
    duration_in_microseconds =
        (jlong)(format_context->duration / micro_seconds_base);
#ifdef DEBUG
  ...
  if (stream->nb_frames != 0 && duration_in_microseconds <= 0 &&
      stream->codecpar->sample_rate > 0) {
    duration_in_microseconds =
        stream->nb_frames * 1000000L / stream->codecpar->sample_rate;
#ifdef DEBUG
...
static jfloat get_frame_rate(AVStream *stream, jlong duration) {
  ...
  if (frame_rate <= 0 && stream->codecpar->frame_size > 0 &&
      stream->codecpar->sample_rate > 0) {
    frame_rate = (jfloat)stream->codecpar->sample_rate /
                 (jfloat)stream->codecpar->frame_size;
  }

  if (frame_rate <= 0 && stream->codecpar->frame_size == 0 &&
      is_pcm(stream->codecpar->codec_id) && stream->codecpar->sample_rate > 0) {
    frame_rate = (jfloat)stream->codecpar->sample_rate;
  }
  ...
  if (stream->codecpar->frame_size == 0 && is_pcm(stream->codecpar->codec_id) &&
      stream->codecpar->sample_rate > 0) {
 ...
 */
static jobject create_ffaudiofileformat(JNIEnv *env, jstring url, jint codecId,
                                        jfloat sampleRate, jint sampleSize,
                                        jint channels, jint frame_size,
                                        jfloat frame_rate, jint frame_length,
                                        jboolean big_endian, jlong duration,
  ...
  return (*env)->NewObject(env, ffAudioFileFormat_class, ffAudioFileFormat_MID,
                           url, codecId, sampleRate, sampleSize, channels,
                           frame_size, frame_rate, frame_length, big_endian,
                           duration, bitRate, vbr, encrypted);
}

static int create_ffaudiofileformats(JNIEnv *env,
                                     AVFormatContext *format_context,
                                     jobjectArray *array, jstring url) {
  int res = 0;
  int pcm = 0;
  jlong duration_in_microseconds = -1;
  jfloat frame_rate = -1;
  ...
  if (audio_stream_count == 0) {
    throwUnsupportedAudioFileExceptionIfError(env, -1,
                                              "Failed to find audio stream");
    goto bail;
  }

  // create output array
  *array = (*env)->NewObjectArray(
      env, audio_stream_count,
      (*env)->FindClass(env, "javax/sound/sampled/AudioFileFormat"), NULL);
  if (*array == NULL) {
    goto bail;
  }
  ...
  for (i = 0; i < format_context->nb_streams; i++) {
    ...
    if (stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
      ...
      if (pcm) {
        frame_size = (stream->codecpar->bits_per_coded_sample / 8) *
      ...
      } else if (duration_in_microseconds > 0 && pcm &&
                 stream->codecpar->sample_rate > 0) {
        frame_length =
            (jint)round(duration_in_microseconds *
                        stream->codecpar->sample_rate / 1000. / 1000.);
      } else if (duration_in_microseconds > 0 &&
                 stream->codecpar->frame_size > 0 &&
                 stream->codecpar->sample_rate > 0) {
        frame_length = (jint)round(
            duration_in_microseconds * stream->codecpar->sample_rate /
            stream->codecpar->frame_size / 1000. / 1000.);
        // re-estimat duration, based on non-PCM frame_size
        duration_in_microseconds = (jlong)(1000. * 1000. * frame_length *
      ...
      sample_size = sample_size <= 0 ? -1 : sample_size;
      ...
      audio_format = create_ffaudiofileformat(
          env, url, stream->codecpar->codec_id,
          (jfloat)stream->codecpar->sample_rate, sample_size,
          stream->codecpar->ch_layout.nb_channels, frame_size, frame_rate,
          frame_length, big_endian, duration_in_microseconds,
          stream->codecpar->bit_rate, vbr, encrypted);

      (*env)->SetObjectArrayElement(env, *array, audio_stream_number,
                                    audio_format);
      audio_stream_number++;
    }
  }
 ...
 */
JNIEXPORT jobjectArray JNICALL
Java_com_tagtraum_ffsampledsp_FFAudioFileReader_getAudioFileFormatsFromBuffer(
    JNIEnv *env, jobject instance, jobject byte_buffer) {
  int res = 0;
  AVFormatContext *format_context = NULL;
  jobjectArray array = NULL;
  ...
  callbackBuffer =
      (unsigned char *)av_malloc(CALLBACK_BUFFERSIZE * sizeof(uint8_t));
  if (!callbackBuffer) {
    res = AVERROR(ENOMEM);
  ...
  if (!io_context) {
    av_free(callbackBuffer);
    callbackBuffer = NULL;
    res = AVERROR(ENOMEM);
    throwIOExceptionIfError(env, res, "Could not allocate custom IO context.");
    goto bail;
  }
  // we didn't supply a seek function in avio_alloc_context, so we need to make
  // sure we don't seek..
