...
static void init_ids(JNIEnv *env) {
  if (getSampleRate_MID == NULL) {
    ...
    encoding_class =
        (*env)->FindClass(env, "javax/sound/sampled/AudioFormat$Encoding");

    getEncoding_MID =
        (*env)->GetMethodID(env, audioFormat_class, "getEncoding",
                            "()Ljavax/sound/sampled/AudioFormat$Encoding;");
    getSampleRate_MID =
        (*env)->GetMethodID(env, audioFormat_class, "getSampleRate", "()F");
    getFrameSize_MID =
        (*env)->GetMethodID(env, audioFormat_class, "getFrameSize", "()I");
    getSampleSizeInBits_MID = (*env)->GetMethodID(env, audioFormat_class,
                                                  "getSampleSizeInBits", "()I");
    getChannels_MID =
        (*env)->GetMethodID(env, audioFormat_class, "getChannels", "()I");
    isBigEndian_MID =
        (*env)->GetMethodID(env, audioFormat_class, "isBigEndian", "()Z");
    toString_MID = (*env)->GetMethodID(env, encoding_class, "toString",
                                       "()Ljava/lang/String;");
  }
}
 ...
 *
 * @param env           JNIEnv
 * @param object        stream instance this call stems from, i.e. a
 * FFCodecInputStream
 * @param target_format target AudioFormat
 * @param aio_pointer   Pointer to the FFAudioIO struct of the
 * FFNativePeerInputStream that opened the file/stream
 * @return pointer to the FFAudioIO struct that was given as parameter
 */
JNIEXPORT jlong JNICALL Java_com_tagtraum_ffsampledsp_FFCodecInputStream_open(
    JNIEnv *env, jobject object, jobject target_format, jlong aio_pointer) {
  int res = 0;
  ...
  jfloat sample_rate =
      (*env)->CallFloatMethod(env, target_format, getSampleRate_MID);
  jint sample_size_in_bits =
      (*env)->CallIntMethod(env, target_format, getSampleSizeInBits_MID);
  jint channels = (*env)->CallIntMethod(env, target_format, getChannels_MID);
  jboolean big_endian =
      (*env)->CallBooleanMethod(env, target_format, isBigEndian_MID);
  jobject encoding =
      (*env)->CallObjectMethod(env, target_format, getEncoding_MID);
  jstring jencoding_name =
      (jstring)(*env)->CallObjectMethod(env, encoding, toString_MID);

  const char *encoding_name =
      (*env)->GetStringUTFChars(env, jencoding_name, NULL);
  ...
  if (sample_size_in_bits <= 8) {
    ...
    fprintf(stderr, "Will use 64 bit PCM_FLOAT even though it might not have "
                    "been desired.\n");
    out_sample_fmt = AV_SAMPLE_FMT_DBL;
  }

  if (aio->stream->codecpar->ch_layout.nb_channels == channels) {
    av_channel_layout_copy(&out_ch_layout, &aio->stream->codecpar->ch_layout);
  } else if (channels == 1) {
  ...
  av_opt_set_chlayout(aio->swr_context, "in_chlayout",
                      &aio->stream->codecpar->ch_layout, 0);
  av_opt_set_int(aio->swr_context, "in_sample_rate",
                 aio->stream->codecpar->sample_rate, 0);
  // custom stuff
  av_opt_set_chlayout(aio->swr_context, "out_chlayout", &out_ch_layout, 0);
  av_opt_set_int(aio->swr_context, "out_sample_rate", (int)round(sample_rate),
                 0);
  ...
  // re-adjust encoder
  encoder = ff_find_encoder(out_sample_fmt, sample_size_in_bits, big_endian,
                            is_signed);
  if (!encoder) {
    res = AVERROR(EINVAL);
