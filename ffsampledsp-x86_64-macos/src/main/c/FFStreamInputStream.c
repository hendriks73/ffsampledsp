...
static void init_ids(JNIEnv *env, jobject stream) {
  if (!fillReadBuffer_MID || !readBuffer_FID) {
    ...
    readBuffer_FID = (*env)->GetFieldID(env, streamClass, "readBuffer",
                                        "Ljava/nio/ByteBuffer;");
    fillReadBuffer_MID =
        (*env)->GetMethodID(env, streamClass, "fillReadBuffer", "()I");
  }
}
 ...
 * @return          number of bytes read or a negative number in case of an
 * error
 */
static int read_callback(void *opaque, uint8_t *buf, int size) {
  ...
  available_data = (int)(*aio->env)->CallIntMethod(aio->env, aio->java_instance,
                                                   fillReadBuffer_MID);
  if (available_data > size) {
    res = -1;
    throwIOExceptionIfError(
        aio->env, 1, "Available data must not be larger than callback buffer.");
    goto bail;
  }
  ...
  read_buffer =
      (*aio->env)->GetObjectField(aio->env, aio->java_instance, readBuffer_FID);
  if (!read_buffer) {
    res = -1;
  ...
  java_buffer =
      (uint8_t *)(*aio->env)->GetDirectBufferAddress(aio->env, read_buffer);
  if (!java_buffer) {
    res = -1;
    throwIOExceptionIfError(aio->env, 1,
                            "Failed to get address for read buffer.");
    goto bail;
  }
...
Java_com_tagtraum_ffsampledsp_FFStreamInputStream_fillNativeBuffer(
    JNIEnv *env, jobject stream, jlong aio_pointer) {
...
/**
 * Creates the FFAudioIO, custom AVIOContext etc for reading data from the
 * stream.
 *
 * @param env       JNIEnv
 ...
 */
JNIEXPORT jlong JNICALL Java_com_tagtraum_ffsampledsp_FFStreamInputStream_open(
    JNIEnv *env, jobject stream, jint streamIndex) {
 ...
 */
JNIEXPORT void JNICALL Java_com_tagtraum_ffsampledsp_FFStreamInputStream_close(
    JNIEnv *env, jobject stream, jlong aio_pointer) {
