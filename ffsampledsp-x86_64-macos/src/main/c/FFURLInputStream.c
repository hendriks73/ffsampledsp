...
JNIEXPORT jlong JNICALL Java_com_tagtraum_ffsampledsp_FFURLInputStream_open(
    JNIEnv *env, jobject stream, jstring url, jbyteArray urlBytes,
    jint streamIndex, jint fileBufferSize) {
...

  {
    ...
    if (!input_url) {
      ...
      res = AVERROR(ENOMEM);
      throwIOExceptionIfError(env, res, "Failed to allocate URL buffer");
      goto bail;
    }
...
Java_com_tagtraum_ffsampledsp_FFURLInputStream_isSeekable(JNIEnv *env,
                                                          jobject stream,
                                                          jlong aio_pointer) {
  FFAudioIO *aio = (FFAudioIO *)(intptr_t)aio_pointer;
 ...
 */
JNIEXPORT void JNICALL Java_com_tagtraum_ffsampledsp_FFURLInputStream_seek(
    JNIEnv *env, jobject stream, jlong aio_pointer, jlong microseconds) {
  FFAudioIO *aio = (FFAudioIO *)(intptr_t)aio_pointer;
  int res = 0;
  ...
  seek_target =
      av_rescale_q(seek_target, AV_TIME_BASE_Q, aio->stream->time_base);
#ifdef DEBUG
  fprintf(stderr,
          "Current Timestamp = %" PRId64 ", seek_target = %" PRId64 "\n",
          current_timestamp, seek_target);
#endif
  res =
      av_seek_frame(aio->format_context, aio->stream_index, seek_target,
                    current_timestamp > seek_target ? AVSEEK_FLAG_BACKWARD : 0);
  if (res < 0) {
  ...
  // set timestamp to seek_target, since that's hopefully now our current
  // timestamp..
 ...
 */
JNIEXPORT void JNICALL Java_com_tagtraum_ffsampledsp_FFURLInputStream_close(
    JNIEnv *env, jobject stream, jlong aio_pointer) {
