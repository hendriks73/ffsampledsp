 ...
 * of the <a
 * href="http://docs.oracle.com/javase/8/docs/api/javax/sound/sampled/spi/package-summary.html">javax.sound.sampled.spi</a>
 * interfaces.
 * <br/>
 * Its main purpose is to decode audio from various formats at high speed.
 * FFSampledSP can support pretty much all formats supported by <a
 * href="https://www.ffmpeg.org">FFmpeg</a> as support for them has been
 * compiled into the used FFmpeg libs. <br/> This is the source documentation
 * for the native part of the library. You can find the documentation for the
 * Java part <a href="../apidocs/index.html">here</a>.
 */

#include <jni.h>
#include <math.h>
// see
// http://stackoverflow.com/questions/4585847/g-linking-error-on-mac-while-compiling-ffmpeg
// or
// http://ffmpeg.org/trac/ffmpeg/wiki/Including%20FFmpeg%20headers%20in%20a%20C%2B%2B%20application
...
/**
 * Central context representing the native peer to the Java
 * FFNativePeerInputStream object.
 */
typedef struct {
  // general stuff
 ...
 *
 * JNI's GetStringUTFChars returns Modified UTF-8 (CESU-8), which encodes
 * supplementary characters (U+10000 and above, e.g. emoji) as two 3-byte
 * sequences (6 bytes total) instead of the single 4-byte sequence used by
 * standard UTF-8. File systems on macOS and Linux use standard UTF-8 for file
 * names, so paths containing emoji passed through GetStringUTFChars will not
 * match the file on disk.
 *
 * This function calls java.lang.String.getBytes("UTF-8") via JNI, which always
 * returns standard UTF-8 bytes regardless of the characters involved.
 *
 * The caller must free() the returned buffer when no longer needed.
 *
 * @param env           JNI environment
 * @param java_string   Java String object
 * @return              Newly allocated null-terminated standard UTF-8 C string,
 * or NULL on error
 */
